...
/**
 * Returnerer riktig PARTER-liste for en bane basert på antall spillere.
 * @param {number} antall  — 4 eller 5
 * @returns {Array}
 */
...
function visHjemStatus() {
  ...
  if (dot) {
    dot.classList.toggle('aktiv', harOkt);
  }

  if (harOkt) {
    if (tekst) tekst.textContent = `🟢 Økt pågår`;
...
function lagSpillerHTML(s, erAktiv, erVente) {
  ...
  const hake = (erAktiv || erVente)
    ...
    : '';
  // Bruker data-id for sikker ID-referanse (unngår potensielle anførselstegn i navn)
  // Legg til nivå-klasse på spiller-elementet
  const nivaaKl = getNivaaKlasse(rating);
  if (!erAktiv && !erVente) kl += ' ' + nivaaKl;
...
async function startTrening() {
  ...
  // Fordel spillere på baner — støtter 4 og 5 spillere per bane
  const baneOversikt = fordelBaner(valgte, app.antallBaner);
    ...
    return `<div class="kort">
      <div class="kort-hode">
        <div style="display:flex;align-items:baseline;gap:8px;flex-wrap:wrap">
          ...
          <span style="font-size:14px;text-transform:uppercase;color:var(--muted2);letter-spacing:1.5px">Bane ${erForst&&!bane.erSingel?'🏆':erSistB&&!app.er6SpillerFormat?'🔻':''}</span>
...
async function bekreftNesteRunde() {
  ...
  try {
    ...
    // ══════════════════════════════════════════
    // 6-SPILLER ROTASJONSFORMAT (resultatbasert)
    // ══════════════════════════════════════════
    if (app.er6SpillerFormat) {
      const mp       = app.poengPerKamp ?? 15;
  ...
  try {
    ...
    fullforteRunder.forEach(rundeNr => {
      baneNrListe.forEach(baneNr => {
        const kamper = alleKamper.filter(k => k.rundeNr === rundeNr && k.baneNr === baneNr && harPoeng(k));
    ...
    // ── Elo-rating per kamp over hele økten ──────────────
    // Bygg spillerliste med startrating fra tsMap
    const spillereListe = sluttrangering.map(s => ({
      id:     s.spillerId,
      rating: tsMap[s.spillerId]?.ratingVedStart ?? STARTRATING,
    }));

    const eloResultat = beregnEloForOkt(alleKamper, spillereListe);
    ...
    app.ratingEndringer = sluttrangering.map(s => {
      ...
      const endring  = elo.endring;
      const nyRating = elo.nyRating;
      return { ...s, ratingVedStart: elo.startRating, endring, nyRating };
    });
    ...
    app.ratingEndringer.forEach(r => {
      ...
      batch.set(doc(collection(db, SAM.RESULTATER)), {
        treningId: app.treningId, spillerId: r.spillerId,
        spillerNavn: r.navn ?? 'Ukjent', sluttPlassering: r.sluttPlassering,
        ratingFor: r.ratingVedStart, ratingEtter: r.nyRating,
        ratingEndring: r.endring, dato: serverTimestamp(),
      });
      batch.set(doc(collection(db, SAM.HISTORIKK)), {
        spillerId: r.spillerId, treningId: app.treningId,
        ratingFor: r.ratingVedStart, ratingEtter: r.nyRating,
        endring: r.endring, plassering: r.sluttPlassering, dato: serverTimestamp(),
      });
